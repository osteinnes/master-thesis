...
int main()
{
    ...
    // Enter IP adress of Lidar connection
    char ip[20];
    ...
    int times = 0;
    printf("Enter measurement times (0 for continuous): ");
    setbuf(stdin, NULL);
    std::cin >> times;
    if(!benewake::startMeasurement(sd, times))
    {
    ...
    while((nFrame < times || times == 0) && gRun)
    {
        ...
        else
        {
            ...
            // Save results to files in .xyzrgb format
            outFile << "Frame count: " << nFrame << std::endl;
            outFile << "Distance:\n";
            for(int i = 0; i < h; i++)
            {
                for(int j = 0; j < w; j++)
            ...
            std::ofstream outP("data" + std::to_string(nFrame) + ".xyzrgb");
            
	    // Fetch point cloud
            benewake::getPointCloud(distData, coord_x, coord_y, coord_z);
            
            // Iterate, place xyz coordinates and scale intensity as color value
            for(int i=0; i<(sizeof(coord_x)/sizeof(*coord_x)); i++)
                outP << coord_x[i] << " " << coord_y[i] << " " << coord_z[i] << " " << (1.0f*(ampData[i]/1590.0f)) << " " << 	(1.0f*(ampData[i]/1590.0f)) << " " << (1.0f*(ampData[i]/1590.0f)) << "\n" << std::endl;
